...
if($desdemenos!='e'){//EL RANGO DESDE NO DEBE SER MAYOR AL HASTA		  
  ...
  $updateSQL = sprintf("UPDATE Tbl_numeracion SET existeTiq_n='1' WHERE int_op_n=%s",
                       GetSQLValueString($_POST['int_op_tn'], "int"));
  mysql_select_db($database_conexion1, $conexion1);
...
mysql_select_db($database_conexion1, $conexion1);
$query_op = sprintf("SELECT * FROM Tbl_numeracion WHERE int_op_n=%s ",$colname_op);
$op = mysql_query($query_op, $conexion1) or die(mysql_error());
$row_op = mysql_fetch_assoc($op);
$totalRows_op = mysql_num_rows($op);

$colname_paquete_rm = "-1";
if (isset($_GET['id_op'])) {
  $colname_paquete_rm  = (get_magic_quotes_gpc()) ? $_GET['id_op'] : addslashes($_GET['id_op']);
}
mysql_select_db($database_conexion1, $conexion1);
$query_tiquete_num = sprintf("SELECT * FROM Tbl_tiquete_numeracionCOPIA WHERE int_op_tn=%s  ORDER BY int_paquete_tn DESC",$colname_paquete_rm );
...
mysql_select_db($database_conexion1, $conexion1);
$query_caja_num = sprintf("SELECT DISTINCT int_caja_tn,int_op_tn FROM Tbl_tiquete_numeracionCOPIA WHERE int_op_tn=%s ORDER BY int_caja_tn  DESC",$colname_caja_rm);
...
$colname_paquete = "-1";
if (isset($_GET['id_op'])) {
  $colname_paquete  = (get_magic_quotes_gpc()) ? $_GET['id_op'] : addslashes($_GET['id_op']);
}
mysql_select_db($database_conexion1, $conexion1);
$query_paquete= sprintf("SELECT * FROM Tbl_tiquete_numeracionCOPIA WHERE int_op_tn=%s  ORDER BY int_hasta_tn DESC",$colname_paquete );
...
function funcion4(){
	var desde_tn=form1.int_desde_tn.value;
	var hasta_tn=form1.int_hasta_tn.value;
	//var int_desde_f=form1.int_desde_f.value;
    //var desde_f = new Array(arr);
    var numeros="0123456789";
	var letr="";num="";a="";b="";c="";d="";e="";f="";g="";h="";
    for(i=0; i<desde_tn.length; i++){
	   f = desde_tn[i+1];
      if (numeros.indexOf(desde_tn.charAt(i),0)!=-1){		  
         b+=desde_tn[i]
      }
	 
   }
   
   for(x=0; x<hasta_tn.length; x++){
	   g = hasta_tn[x+1];
      if (numeros.indexOf(hasta_tn.charAt(x),0)!=-1){
		 c+=hasta_tn[x]		         
      }	 
   } 
 }   
...
<tr><td id="nombreusuario"><?php echo $row_usuario['nombre_usuario']; ?></td>
  ...
  <tr>
    <td colspan="2" align="center" id="linea1">
      <form action="<?php echo $editFormAction; ?>" method="POST" name="form1" onSubmit="return validacion_select();return funcion();">
        <table align="center" id="tabla35">
          ...
          <tr>
            <td id="dato1"><strong>PAQUETE N&deg;:</strong><span class="rojo_inteso"><?php $num=$row_tiquete_num['int_paquete_tn']+1; //echo $num;?></span>
              <input name="int_paquete_tn" style="width:50px;" type="number" id="int_paquete_tn" value="<?php echo $num; ?>" maxlength="5"></td>
            <td id="dato3"><strong>CAJA N&deg; : <span class="rojo_inteso">
                      <!--if($num==$row_op['int_paquete_n']){$num2=$num2+1;}-->
                    <?php $num2=$row_tiquete_num['int_caja_tn']+1; //echo $num2;?></span>
                <input name="int_caja_tn" id="int_caja_tn" style="width:50px;" type="number" value="<?php echo $num2; //}?>">
                </span></strong></td>
            <td id="dato3">&nbsp;</td>
            </tr>
          <tr>
          ...
          <tr>
            ...
            <td colspan="2" ><input type="number" name="int_op_tn" id="pswd" value="<?php echo $row_op['int_op_n'];?>" readonly onFocus="onFocus"></td>
              </tr>
          <tr>
            <td id="fuente1">BOLSAS millar/und</td>
            <td colspan="2" ><input type="number" name="int_bolsas_tn" id="pswd" min="0" value="<?php echo $row_op['int_bolsas_n'];?>" readonly></td>
          </tr>
      
          <tr>
            <td id="fuente1">UNIDADES X CAJA</td>
            <td colspan="2" ><input type="number" name="int_undxcaja_tn" readonly id="int_undxcaja_tn" min="0" value="<?php echo $row_op['int_undxcaja_n'];?>"></td> 
          </tr>
          <tr>
            <td id="fuente1">UNIDADES  X PAQ.</td>
            <td colspan="2" ><input type="number" name="int_undxpaq_tn" readonly id="pswd"min="0" value="<?php echo $row_op['int_undxpaq_n'];?>" ></td>
            </tr>
          <tr>
            <td id="fuente1"><strong>DESDE</strong></td>
            <td colspan="2" ><?php $paq=$row_op['int_undxpaq_n'];$desde=$row_paquete['int_desde_tn'];if($desde==''){$desde=$row_op['int_desde_n'];}else{$desde=$row_paquete['int_hasta_tn']+1;}?>
            
            <input type="text" name="int_desde_tn"  id="pswd" value="<?php echo $desde; ?>"min="0" onBlur="conMayusculas(this);" onChange="sumaPaqSelladoAdd(this);" autofocus required></td>
          </tr>
          <tr>
            <td id="fuente1"><strong>HASTA</strong></td>
            <td colspan="2" ><input type="text" name="int_hasta_tn" id="pswd" required value="" min="0"></td>
          </tr>
          <tr>
            ...
            <td colspan="2" id="fuente1"><select name="int_cod_empleado_tn" id="operario" onBlur="if(form1.int_cod_empleado_tn.value=='') { alert('Debe Seleccionar un empleado')}" style="width:157px">
              <option value=""<?php if (!(strcmp("", $row_op['int_cod_empleado_n']))) {echo "selected=\"selected\"";} ?>>Seleccione</option>
              <?php
do {  
?>
              <option value="<?php echo $row_codigo_empleado['codigo_empleado']?>"<?php if (!(strcmp($row_codigo_empleado['codigo_empleado'], $row_op['int_cod_empleado_n']))) {echo "selected=\"selected\"";} ?>><?php echo $row_codigo_empleado['nombre_empleado']." ".$row_codigo_empleado['apellido_empleado']?></option>
              <?php
...
<tr>
            ...
            <td colspan="2" id="fuente1"><select name="int_cod_rev_tn" id="revisor" onBlur="if(form1.int_cod_rev_tn.value=='') { alert('Debe Seleccionar un revisor')}" style="width:157px">
              <option value=""<?php if (!(strcmp("", $row_op['int_cod_rev_n']))) {echo "selected=\"selected\"";} ?>>Seleccione</option>
              <?php
do {  
?>
              <option value="<?php echo $row_codigo_empleado['codigo_empleado']?>"<?php if (!(strcmp($row_codigo_empleado['codigo_empleado'], $row_op['int_cod_rev_n']))) {echo "selected=\"selected\"";} ?>><?php echo $row_codigo_empleado['nombre_empleado']." ".$row_codigo_empleado['apellido_empleado']?></option>
              <?php
} while ($row_codigo_empleado = mysql_fetch_assoc($codigo_empleado));
  $rows = mysql_num_rows($codigo_empleado);
  if($rows > 0) {
...
?>
          ...
          <tr>
            ...
            <td colspan="2" id="fuente1"><strong><?php echo $num. " de " . $row_op['int_paquete_n'];?></strong></td>
          </tr>
          <tr>
            <td colspan="3">            
             </td>
            ...
            <td colspan="3">
			  <!--CONTROS DE PAQUETES-->
			  <?php if($num>$row_op['int_paquete_n']){?>
          ...
          <tr>
            <td colspan="3" id="dato2"><button type="submit" style='width:300px; height:45px' onClick="return funcion2();" >GUARDAR NUMERACION</button></td>
          </tr>
          <tr>
...
<div class="bordesolido" id="ventanas">
  <?php  do { ?>
            <p><a href="javascript:popUp('sellado_control_numeracion_vista.php?id_op=<?php echo $row_tiquete_num['int_op_tn']; ?>&id_tn=<?php echo $row_tiquete_num['id_tn']; ?>','770','300')" target="_top"><?php echo "PAQUETE #: ".$row_tiquete_num['int_paquete_tn']. " DESDE: ".  $row_tiquete_num['int_desde_tn']. " HASTA: ".$row_tiquete_num['int_hasta_tn']?></a>-----<a href="javascript:eliminar_sp('id_tn',<?php echo $row_tiquete_num['id_tn'];?>,'sellado_control_numeracion_edit.php')"><img src="images/por.gif" style="cursor:hand;" alt="ELIMINAR TIQUETE"
