...
<table id="tabla1">
  ...
  <?php do { ?>
	...
	echo $proveedor_p; } ?>
      ...
      <td id="dato1"><a href="rollos_vista.php?id_rollo=<?php echo $row_rollos_oc['id_rollo_ocr']; ?>" target="_top" style="text-decoration:none; color:#000000"><?php $id_rollo=$row_rollos_oc['id_rollo_ocr']; if($id_rollo!='') {
	...
	$ref_prod_rollo=mysql_result($resultrollo,0,'ref_prod_rollo');  
      ...
      <td id="dato2"><a href="referencia_vista.php?id_ref=<?php echo $row_rollos_oc['id_ref_ocr']; ?>" target="_top" style="text-decoration:none; color:#000000"><?php $ref=$row_rollos_oc['id_ref_ocr'];
	  if($ref!=''){	$sqlref="SELECT * FROM Tbl_referencia WHERE id_ref = $ref";
	$resultref= mysql_query($sqlref);
	$numref= mysql_num_rows($resultref);
	if($numref >='1') { $cod_ref=mysql_result($resultref,0,'cod_ref'); } } echo $cod_ref; ?></a></td>
