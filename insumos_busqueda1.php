...
$codigo_insumo = $_GET['codigo_insumo'];
$id_insumo = $_GET['id_insumo'];
$tipo_insumo = $_GET['tipo_insumo'];
$clase = $_GET['clase'];
if($codigo_insumo == '0' && $id_insumo == '0' && $tipo_insumo == '0' && $clase=='0')
{
...
if($codigo_insumo != '0' && $id_insumo == '0' && $tipo_insumo == '0' && $clase=='0')
{
$registros=$conexion->buscarListar("insumo","*","ORDER BY descripcion_insumo ASC ","",$maxRows_registros,$pageNum_registros,"WHERE id_insumo='$codigo_insumo'" );
}
if($codigo_insumo == '0' && $id_insumo != '0' && $tipo_insumo == '0' && $clase=='0')
{
$registros=$conexion->buscarListar("insumo","*","ORDER BY descripcion_insumo ASC ","",$maxRows_registros,$pageNum_registros,"WHERE id_insumo='$id_insumo'" );
}
if($codigo_insumo == '0' && $id_insumo == '0' && $tipo_insumo != '0' && $clase=='0')
{
$registros=$conexion->buscarListar("insumo","*","ORDER BY descripcion_insumo ASC ","",$maxRows_registros,$pageNum_registros,"WHERE tipo_insumo='$tipo_insumo'" );
}
if($codigo_insumo == '0' && $id_insumo == '0' && $tipo_insumo == '0' && $clase!='0')
{
$registros=$conexion->buscarListar("insumo","*","ORDER BY descripcion_insumo ASC ","",$maxRows_registros,$pageNum_registros,"WHERE clase_insumo='$clase'" );
}
...
<tr>
  <td colspan="2" id="fuente2">
    <select name="codigo_insumo" id="codigo_insumo" class="selectsMini">
  <option value="0" <?php if (!(strcmp(0, $_GET['codigo_insumo']))) {echo "selected=\"selected\"";} ?>>CODIGO</option>
  <?php  foreach($row_codigo as $row_codigo ) { ?>
    <option value="<?php echo $row_codigo['id_insumo']?>"<?php if (!(strcmp($row_codigo['id_insumo'], $_GET['codigo_insumo']))) {echo "selected=\"selected\"";} ?>><?php echo $row_codigo['codigo_insumo']?></option>
    <?php }  ?>
  </select>
  <select name="id_insumo" id="id_insumo" class="selectsMini">
    ...
    <?php  foreach($row_lista as $row_lista ) { ?>
      <option value="<?php echo $row_lista['id_insumo']?>"<?php if (!(strcmp($row_lista['id_insumo'], $_GET['id_insumo']))) {echo "selected=\"selected\"";} ?>><?php echo $row_lista['descripcion_insumo']?></option>
      <?php }  ?>
  </select>
  <select name="tipo_insumo" id="tipo_insumo" class="selectsMini">
    <option value="0" <?php if (!(strcmp(0, $_GET['tipo_insumo']))) {echo "selected=\"selected\"";} ?>>TIPO</option>
    <?php  foreach($row_tipo as $row_tipo ) { ?>
      <option value="<?php echo $row_tipo['id_tipo']?>"<?php if (!(strcmp($row_tipo['id_tipo'], $_GET['tipo_insumo']))) {echo "selected=\"selected\"";} ?>><?php echo $row_tipo['nombre_tipo']?></option>
      <?php } ?>
  </select> 
  <select name="clase" id="clase" class="selectsMini">
    ...
    <?php  foreach($row_clase as $row_clase ) { ?>
      <option value="<?php echo $row_clase['id_clase']?>"<?php if (!(strcmp($row_tipo['id_clase'], $_GET['clase']))) {echo "selected=\"selected\"";} ?>><?php echo $row_clase['nombre_clase']?></option>
      <?php }  ?>
  </select>
