...
//Filtra todos vacios
if($str_numero_oc == '0' && $id_c == '0' && $nit_c == '0'&& $estado_oc== '0')
{
...
//Filtra str_numero_oc lleno
if($str_numero_oc != '0' && $id_c == '0' && $nit_c == '0'&& $estado_oc== '0')
{
$row_ordenes_compra = $conexion->buscarListar("tbl_orden_compra","*","GROUP BY  str_numero_oc ORDER BY  b_estado_oc, fecha_ingreso_oc DESC","WHERE str_numero_oc='$str_numero_oc' AND b_borrado_oc='1' ",$maxRows_ordenes_compra,$pageNum_ordenes_compra,"" );
//$query_ordenes_compra = "SELECT * FROM Tbl_orden_compra WHERE str_numero_oc='$str_numero_oc' AND b_borrado_oc='1' GROUP BY  str_numero_oc ORDER BY  b_estado_oc, fecha_ingreso_oc DESC";
}
//Filtra id_c lleno
if($str_numero_oc == '0' && $id_c != '0' && $nit_c == '0'&& $estado_oc== '0')
{
$row_ordenes_compra = $conexion->buscarListar("tbl_orden_compra","*","GROUP BY str_numero_oc ORDER BY  b_estado_oc,  fecha_ingreso_oc DESC","WHERE id_c_oc='$id_c' AND b_borrado_oc='1'",$maxRows_ordenes_compra,$pageNum_ordenes_compra,"" );
//$query_ordenes_compra = "SELECT * FROM Tbl_orden_compra WHERE id_c_oc='$id_c' AND b_borrado_oc='1' GROUP BY str_numero_oc ORDER BY  b_estado_oc,  fecha_ingreso_oc DESC";
}
//Filtra nit_c lleno
if($str_numero_oc == '0' && $id_c == '0' && $nit_c != '0'&& $estado_oc== '0')
{
$row_ordenes_compra = $conexion->buscarListar("tbl_orden_compra","*","GROUP BY str_numero_oc ORDER BY","WHERE str_nit_oc='$nit_c' AND b_borrado_oc='1'",$maxRows_ordenes_compra,$pageNum_ordenes_compra,"" );
//$query_ordenes_compra = "SELECT * FROM Tbl_orden_compra WHERE str_nit_oc='$nit_c' AND b_borrado_oc='1' GROUP BY str_numero_oc ORDER BY  b_estado_oc,  fecha_ingreso_oc DESC";
}
//Filtra estado_oc lleno
if($str_numero_oc == '0' && $id_c == '0' && $nit_c == '0'&& $estado_oc!= '0')
{
$row_ordenes_compra = $conexion->buscarListar("tbl_orden_compra","*","GROUP BY str_numero_oc ORDER BY  b_estado_oc, fecha_ingreso_oc DESC","WHERE str_nit_oc='$nit_c' AND b_borrado_oc='1'",$maxRows_ordenes_compra,$pageNum_ordenes_compra,"" );
//$query_ordenes_compra = "SELECT * FROM Tbl_orden_compra WHERE b_estado_oc='$estado_oc' AND b_borrado_oc='1' GROUP BY str_numero_oc ORDER BY  b_estado_oc, fecha_ingreso_oc DESC";
}
//Filtra pendientes lleno
if($str_numero_oc == '0' && $id_c == '0' && $nit_c == '0'&& $estado_oc == '0' && $pendiente != '0')
{
$row_ordenes_compra = $conexion->buscarListar("Tbl_orden_compra,Tbl_items_ordenc"," *,SUM(Tbl_items_ordenc.int_cantidad_rest_io) AS restante","GROUP BY Tbl_orden_compra.str_numero_oc ORDER BY Tbl_orden_compra.b_estado_oc, Tbl_orden_compra.fecha_ingreso_oc DESC","WHERE Tbl_items_ordenc.int_cantidad_rest_io $pendiente '0.00' AND Tbl_orden_compra.id_pedido=Tbl_items_ordenc.id_pedido_io AND Tbl_orden_compra.b_borrado_oc='0' ",$maxRows_ordenes_compra,$pageNum_ordenes_compra,"" );
//$query_ordenes_compra = "SELECT *,SUM(Tbl_items_ordenc.int_cantidad_rest_io) AS restante FROM Tbl_orden_compra,Tbl_items_ordenc WHERE Tbl_items_ordenc.int_cantidad_rest_io $pendiente '0.00' AND Tbl_orden_compra.id_pedido=Tbl_items_ordenc.id_pedido_io AND Tbl_orden_compra.b_borrado_oc='0' GROUP BY Tbl_orden_compra.str_numero_oc ORDER BY Tbl_orden_compra.b_estado_oc, Tbl_orden_compra.fecha_ingreso_oc DESC";
}
//Filtra cliente y pendientes lleno
if($str_numero_oc == '0' && $id_c != '0' && $nit_c == '0'&& $estado_oc == '0' && $pendiente != '0')
{
$row_ordenes_compra = $conexion->buscarListar("Tbl_orden_compra,Tbl_items_ordenc"," *,SUM(Tbl_items_ordenc.int_cantidad_rest_io) AS restante","ORDER BY Tbl_orden_compra.b_estado_oc, Tbl_orden_compra.fecha_ingreso_oc DESC","WHERE Tbl_orden_compra.id_c_oc='$id_c' AND Tbl_items_ordenc.int_cantidad_rest_io $pendiente '0.00' AND Tbl_orden_compra.id_pedido=Tbl_items_ordenc.id_pedido_io AND Tbl_orden_compra.b_borrado_oc='0' ",$maxRows_ordenes_compra,$pageNum_ordenes_compra,"" );
...
if($str_numero_oc == '0' && $id_c == '0' && $nit_c != '0'&& $estado_oc == '0' && $pendiente != '0')
{
$row_ordenes_compra = $conexion->buscarListar("Tbl_orden_compra,Tbl_items_ordenc"," *,SUM(Tbl_items_ordenc.int_cantidad_rest_io) AS restante","GROUP BY Tbl_orden_compra.str_numero_oc ORDER BY Tbl_orden_compra.b_estado_oc, Tbl_orden_compra.fecha_ingreso_oc DESC","WHERE Tbl_orden_compra.str_nit_oc='$nit_c' AND Tbl_items_ordenc.int_cantidad_rest_io $pendiente '0.00' AND Tbl_orden_compra.id_pedido=Tbl_items_ordenc.id_pedido_io AND Tbl_orden_compra.b_borrado_oc='0' ",$maxRows_ordenes_compra,$pageNum_ordenes_compra,"" );
//$query_ordenes_compra = "SELECT *,SUM(Tbl_items_ordenc.int_cantidad_rest_io) AS restante FROM Tbl_orden_compra,Tbl_items_ordenc WHERE Tbl_orden_compra.str_nit_oc='$nit_c' AND Tbl_items_ordenc.int_cantidad_rest_io $pendiente '0.00' AND Tbl_orden_compra.id_pedido=Tbl_items_ordenc.id_pedido_io AND Tbl_orden_compra.b_borrado_oc='0' GROUP BY Tbl_orden_compra.str_numero_oc ORDER BY Tbl_orden_compra.b_estado_oc, Tbl_orden_compra.fecha_ingreso_oc DESC";
}
//Filtra O.C y pendientes lleno
if($str_numero_oc != '0' && $id_c == '0' && $nit_c == '0'&& $estado_oc == '0' && $pendiente != '0')
{
$row_ordenes_compra = $conexion->buscarListar("Tbl_orden_compra,Tbl_items_ordenc"," *,SUM(Tbl_items_ordenc.int_cantidad_rest_io) AS restante","GROUP BY Tbl_orden_compra.str_numero_oc ORDER BY Tbl_orden_compra.b_estado_oc, Tbl_orden_compra.fecha_ingreso_oc DESC","WHERE Tbl_orden_compra.str_numero_oc='$str_numero_oc' AND Tbl_items_ordenc.int_cantidad_rest_io $pendiente '0.00' AND Tbl_orden_compra.id_pedido=Tbl_items_ordenc.id_pedido_io AND Tbl_orden_compra.b_borrado_oc='0' ",$maxRows_ordenes_compra,$pageNum_ordenes_compra,"" );
//$query_ordenes_compra = "SELECT *,SUM(Tbl_items_ordenc.int_cantidad_rest_io) AS restante FROM Tbl_orden_compra,Tbl_items_ordenc WHERE Tbl_orden_compra.str_numero_oc='$str_numero_oc' AND Tbl_items_ordenc.int_cantidad_rest_io $pendiente '0.00' AND Tbl_orden_compra.id_pedido=Tbl_items_ordenc.id_pedido_io AND Tbl_orden_compra.b_borrado_oc='0' GROUP BY Tbl_orden_compra.str_numero_oc ORDER BY Tbl_orden_compra.b_estado_oc, Tbl_orden_compra.fecha_ingreso_oc DESC";
}
//Filtra estado y pendientes lleno
if($str_numero_oc == '0' && $id_c == '0' && $nit_c == '0'&& $estado_oc != '0' && $pendiente != '0')
{
$row_ordenes_compra = $conexion->buscarListar("Tbl_orden_compra,Tbl_items_ordenc"," *,SUM(Tbl_items_ordenc.int_cantidad_rest_io) AS restante","GROUP BY Tbl_orden_compra.str_numero_oc ORDER BY Tbl_orden_compra.b_estado_oc, Tbl_orden_compra.fecha_ingreso_oc DESC","WHERE Tbl_orden_compra.id_c_oc='$id_c' AND Tbl_orden_compra.str_numero_oc='$str_numero_oc' AND Tbl_orden_compra.b_estado_oc='$estado_oc'Tbl_items_ordenc.int_cantidad_rest_io $pendiente '0.00' AND Tbl_orden_compra.id_pedido=Tbl_items_ordenc.id_pedido_io AND Tbl_orden_compra.b_borrado_oc='0' ",$maxRows_ordenes_compra,$pageNum_ordenes_compra,"" );
...
//Filtra todos SIN NIT lleno
if($str_numero_oc != '0' && $id_c != '0' && $nit_c == '0'&& $estado_oc != '0' && $pendiente != '0')
{
$row_ordenes_compra = $conexion->buscarListar("Tbl_orden_compra,Tbl_items_ordenc"," *,SUM(Tbl_items_ordenc.int_cantidad_rest_io) AS restante","GROUP BY Tbl_orden_compra.str_numero_oc ORDER BY Tbl_orden_compra.b_estado_oc, Tbl_orden_compra.fecha_ingreso_oc DESC","WHERE Tbl_orden_compra.id_c_oc='$id_c' AND Tbl_orden_compra.str_numero_oc='$str_numero_oc' AND Tbl_orden_compra.b_estado_oc='$estado_oc'Tbl_items_ordenc.int_cantidad_rest_io $pendiente '0.00' AND Tbl_orden_compra.id_pedido=Tbl_items_ordenc.id_pedido_io AND Tbl_orden_compra.b_borrado_oc='0' ",$maxRows_ordenes_compra,$pageNum_ordenes_compra,"" );
//$query_ordenes_compra = "SELECT *,SUM(Tbl_items_ordenc.int_cantidad_rest_io) AS restante FROM Tbl_orden_compra,Tbl_items_ordenc WHERE Tbl_orden_compra.id_c_oc='$id_c' AND Tbl_orden_compra.str_numero_oc='$str_numero_oc' AND Tbl_orden_compra.b_estado_oc='$estado_oc'Tbl_items_ordenc.int_cantidad_rest_io $pendiente '0.00' AND Tbl_orden_compra.id_pedido=Tbl_items_ordenc.id_pedido_io AND Tbl_orden_compra.b_borrado_oc='0' GROUP BY Tbl_orden_compra.str_numero_oc ORDER BY Tbl_orden_compra.b_estado_oc, Tbl_orden_compra.fecha_ingreso_oc DESC";
}
  ...
  <td colspan="2" id="fuente2">
    ...
    <select class="selectsMedio busqueda" name="str_numero_oc" id="str_numero_oc">
       <option value="0"<?php if (!(strcmp(0, $_GET['str_numero_oc']))) {echo "selected=\"selected\"";} ?>>O.C.</option>
        ...
        <option value="<?php echo $row_lista['str_numero_oc']?>"<?php if (!(strcmp($row_lista['str_numero_oc'], $_GET['str_numero_oc']))) {echo "selected=\"selected\"";} ?>><?php echo $row_lista['str_numero_oc']?></option>
    <?php } ?>
    </select>
    ...
    <select class="selectsMedio busqueda" name="id_c" id="id_c">
       <option value="0"<?php if (!(strcmp(0, $_GET['id_c']))) {echo "selected=\"selected\"";} ?>>CLIENTE</option>
        ...
        <option value="<?php echo $row_proveedores['id_c']?>"<?php if (!(strcmp($row_proveedores['id_c'], $_GET['id_c']))) {echo "selected=\"selected\"";} ?>><?php $cad =($row_proveedores['nombre_c']);echo $cad;?></option>
    <?php } ?>
    </select>
    ...
    <select class="selectsMedio busqueda" name="nit_c" id="nit_c">
       <option value="0"<?php if (!(strcmp(0, $_GET['nit_c']))) {echo "selected=\"selected\"";} ?>>NIT</option>
        ...
        <option value="<?php echo $row_ano['nit_c']?>"<?php if (!(strcmp($row_ano['nit_c'], $_GET['nit_c']))) {echo "selected=\"selected\"";} ?>><?php echo $row_ano['nit_c']?></option>
    <?php } ?>
    </select>
...
<table class="table table-bordered table-sm">
  ...
  if($id == '0') { ?>
    ...
    <td colspan="3" id="dato2"><a href="orden_compra_cl_add.php"><img src="images/mas.gif" alt="ADD ORDEN DE COMPRA" title="ADD ORDEN DE COMPRA" border="0" style="cursor:hand;"/></a><a href="orden_compra_cl.php"><img src="images/ciclo1.gif" alt="RESTAURAR" border="0" style="cursor:hand;"/></a></td>
    </tr>  
  <tr id="tr1">
