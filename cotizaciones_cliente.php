...
//$id_c = $row_cliente['nit_c'];//VARIABLE PARA CONSULTA
mysql_select_db($database_conexion1, $conexion1);
...
mysql_select_db($database_conexion1, $conexion1);
$query_ref3 = sprintf("SELECT * FROM Tbl_cotiza_packing WHERE N_referencia_c='%s'",$colname_ver_ref);
$ver_ref3 = mysql_query($query_ref3, $conexion1) or die(mysql_error());
$num7=mysql_num_rows($ver_ref3);

mysql_select_db($database_conexion1, $conexion1);
$query_ref4 = sprintf("SELECT * FROM Tbl_cotiza_laminas WHERE N_referencia_c='%s'",$colname_ver_ref);
$ver_ref4 = mysql_query($query_ref4, $conexion1) or die(mysql_error());
$num8=mysql_num_rows($ver_ref4);

mysql_select_db($database_conexion1, $conexion1);
$query_ref5 = sprintf("SELECT * FROM Tbl_cotiza_materia_p WHERE N_referencia_c='%s'",$colname_ver_ref);
...
$colname_ver_ref_nit= "-1";
if (isset($_GET['ref'])) 
{
  $colname_ver_ref_nit= (get_magic_quotes_gpc()) ? $_GET['ref'] : addslashes($_GET['ref']);
}
mysql_select_db($database_conexion1, $conexion1);
$query_ver_ref_nit = sprintf("SELECT * FROM Tbl_cliente_referencia WHERE N_referencia='%s'",$colname_ver_ref_nit);
...
/*$colname_cliente_ref= "-1";
if (isset($_GET['nit'])) 
{
  $colname_cliente_ref= (get_magic_quotes_gpc()) ? $_GET['nit'] : addslashes($_GET['nit']);
}
mysql_select_db($database_conexion1, $conexion1);
$query_cotizacion_cliente = sprintf("SELECT * FROM Tbl_cliente_referencia, cliente WHERE Tbl_cliente_referencia.Str_nit ='%s' AND Tbl_cliente_referencia.Str_nit = cliente.nit_c",$colname_cliente_ref);
$cotizacion_cliente = mysql_query($query_cotizacion_cliente, $conexion1) or die(mysql_error());
$row_cotizacion_cliente = mysql_fetch_assoc($cotizacion_cliente);
$totalRows_cotizacion_cliente = mysql_num_rows($cotizacion_cliente);*/
//FIN CONSULTA PARA ENVIAR EL NIT CORRESPONDIENTE A LA REFERENCIA
?>
...
<td><table id="tablainterna">
  ...
  <tr>
    <td><table id="tablainterna">
      <tr>
        <td id="fuente10"><form action="cotizaciones_clientes.php" method="GET" name="form1"> 
            REF.
...
do {  
?>
            <option value="<?php echo $row_ref['N_referencia']?>"><?php echo $row_ref['N_referencia']?></option>
            <?php
...
?>      
          ...
          <input type="submit" name="Submit" value="BUSQUEDA" onclick="if(BUSQUEDA.ref.value=='0') { alert('DEBE SELECCIONAR UNA OPCION'); }"/> 
          <input name="nit" type="hidden" value="<?php echo $row_ref_nit['Str_nit'] ?>"> <!--pendiente-->
          </form>  
</td>
        <td id="fuente10"><form name="form1" method="get" action="cotizaciones_clientes.php">        
CLIENTE:
...
{ ?>
      <table id="tablainterna">
        ...
        <tr>
          <td id="fuente7">ANCHO(cm)</td>
		  <?php  for ($l=0;$l<=$num5-1;$l++) { ?>
		  <td id="fuente2"><?php $var=mysql_result($ver_pl,$l,N_ancho); echo $var; ?></td>
		  <?php } ?>
        </tr>
        <tr>
          <td id="fuente7">ALTO(cm)</td>
		  <?php  for ($l=0;$l<=$num5-1;$l++) { ?>
		  <td id="fuente2"> <?php $var=mysql_result($ver_pl,$l,N_alto); echo $var; ?></td> 
		  <?php } ?>
        </tr>
        <tr>
        ...
        <tr>
		  ...
		  <?php  for ($l=0;$l<=$num5-1;$l++) { ?>
          <td id="fuente2"><?php $var=mysql_result($ver_pl,$l,N_calibre); echo $var; ?></td>
		  <?php } ?>
        </tr> 
        <tr>
          <td id="fuente7">INCOTERMS</td>
        ...
        <tr>
          <td id="fuente7">PRECIO($)</td>
		  <?php  for ($l=0;$l<=$num5-1;$l++) { ?>
         <td id="fuente2"><?php $var=mysql_result($ver_pl,$l,Str_moneda); echo $var; ?>/<?php $var=mysql_result($ver_pl,$l,N_precio_vnta); echo $var; ?></td>          <?php } ?>
        </tr>
        ...
        <tr>
          <td id="fuente7">COLORES IMPRESION</td>
		  <?php  for ($l=0;$l<=$num5-1;$l++) { ?>
          <td id="fuente2"><?php $var=mysql_result($ver_pl,$l,N_colores_impresion); echo $var; ?></td>
		  <?php } ?>
        </tr>
        <tr>
          <td id="fuente7">CYRELES</td>
          <?php  for ($l=0;$l<=$num5-1;$l++) { ?>
          <td id="fuente2"><?php $var=mysql_result($ver_pl,$l,B_cyreles);if($var=='1'){ echo "SI"; }else if($var=='0'){echo "NO";} ?></td>
		  <?php } ?>
        </tr>
        <tr>
        ...
        <tr>
         ...
         <?php  for ($l=0;$l<=$num5-1;$l++) { ?>
          <td id="fuente2"><?php $var=mysql_result($ver_pl,$l,Str_lam1); echo $var; ?></td>
		  <?php } ?>
        </tr>
        <tr>
        ...
        <tr>
          <td id="fuente7">PLAZO DE PAGO</td>
          <?php  for ($l=0;$l<=$num5-1;$l++) { ?>
          <td id="fuente2"><?php $var=mysql_result($ver_pl,$l,Str_plazo); echo $var; ?></td>
		  <?php } ?>
        </tr>         
      </table>       
      <?php }?>
...
{ ?>
      <table id="tablainterna">
        ...
        <tr>
          <td id="fuente7">ANCHO(cm) </td>
		  <?php  for ($j=0;$j<=$num3-1;$j++) { ?>
          <td width="544" id="fuente2"><?php $var=mysql_result($ver_lamina,$j,N_ancho); echo $var; ?></td>
		  <?php } ?>
        </tr>
        <tr>
          <td id="fuente7">REPETICION</td>
		  <?php  for ($j=0;$j<=$num3-1;$j++) { ?>
		  <td id="fuente2"><?php $var=mysql_result($ver_lamina,$j,N_repeticion); echo $var; ?></td>
		  <?php } ?>
        </tr>
        <tr>
          <td id="fuente7">CALIBRE (micras) </td>
		  <?php  for ($j=0;$j<=$num3-1;$j++) { ?>
		  <td id="fuente2"><?php $var=mysql_result($ver_lamina,$j,N_calibre); echo $var; ?></td> 
		  <?php } ?>
        </tr>
        <tr>
        ...
        <tr>
          <td id="fuente7">COLORES IMPRESION </td>
		  <?php  for ($j=0;$j<=$num3-1;$j++) { ?>
          <td id="fuente2"><?php $var=mysql_result($ver_lamina,$j,N_colores_impresion); echo $var; ?></td>
		  <?php } ?>
        </tr>
        <tr>
          <td id="fuente7">CYRELES</td>
		  <?php  for ($j=0;$j<=$num3-1;$j++) { ?>
          <td id="fuente2"><?php $var=mysql_result($ver_lamina,$j,B_cyreles);if($var=='1'){ echo "SI"; }else if($var=='0'){echo "NO";} ?></td>
		  <?php } ?>
        </tr>
        <tr>
         <td id="fuente7">CANTIDAD</td>
        ...
        <tr>
		  ...
		  <?php  for ($j=0;$j<=$num3-1;$j++) { ?>
          <td id="fuente2"><?php $var=mysql_result($ver_lamina,$j,N_peso_max); echo $var; ?></td>
		  <?php } ?>
        </tr>
        <tr>
          <td id="fuente7">DIAMETRO MAX.</td>
          <?php  for ($j=0;$j<=$num3-1;$j++) { ?>
          <td id="fuente2"><?php $var=mysql_result($ver_lamina,$j,N_diametro_max); echo $var; ?></td>
		  <?php } ?>
        </tr>
        <tr>
          <td id="fuente7">PRECIO($)</td>
          <?php  for ($j=0;$j<=$num3-1;$j++) { ?>
          <td id="fuente2"><?php $var=mysql_result($ver_lamina,$j,Str_moneda); echo $var; ?>/<?php $var=mysql_result($ver_lamina,$j,N_precio_k); echo $var; ?></td>
		  <?php } ?>
        </tr>
      </table>     
      <?php }?>
...
{ ?>
      <table id="tablainterna">
        ...
        <tr>
		  ...
		  <?php  for ($k=0;$k<=$num4-1;$k++) { ?>
		  <td id="fuente2"><?php $var=mysql_result($ver_materia,$k,N_cantidad); echo $var; ?></td>
		  <?php } ?>
        </tr>
        <tr>
          <td id="fuente7">INCOTERMS</td>
		  <?php  for ($k=0;$k<=$num4-1;$k++) { ?>
		  <td id="fuente2"> <?php $var=mysql_result($ver_materia,$k,Str_incoterms); echo $var; ?></td>
          <?php } ?>
        </tr>
        <tr>
          <td id="fuente7">PRECIO($)</td>
		  <?php  for ($k=0;$k<=$num4-1;$k++) { ?>
          <td id="fuente2"><?php $var=mysql_result($ver_materia,$k,Str_moneda); echo $var; ?>/<?php $var=mysql_result($ver_materia,$k,N_precio_vnta); echo $var; ?></td>
		  <?php } ?>
        </tr>
        <tr>
       ...
       <tr>
		  ...
		  <?php  for ($k=0;$k<=$num4-1;$k++) { ?>
          <td id="fuente2"> <?php $var=mysql_result($ver_materia,$k,Str_unidad_vta); echo $var; ?></td>
          <?php } ?>
        </tr>
       <tr>
          <td id="fuente7">PLAZO DE PAGO</td>
...
<table id="tablainterna" align="center">
  <tr>
    <td id="noprint" align="center"><?php if($_GET['tipo']=='1') { ?> <a href="cotizaciones_clientes.php?Str_nit=<?php echo $_GET['Str_nit']; ?>&N_cotizacion=<?php echo $_GET['N_cotizacion']; ?>"><img src="images/menos.gif" alt="EDICION" border="0" style="cursor:hand;"></a><?php } ?><img src="images/impresor.gif" onclick="window.print();" style="cursor:hand;" alt="IMPRIMIR" /><?php if($_GET['tipo']=='1') { ?><a href="cotizacion_bolsa_add.php"><img src="images/mas.gif" alt="ADD COTIZACION" border="0" style="cursor:hand;"/></a><?php } ?><a href="comercial.php"><img src="images/opciones.gif" style="cursor:hand;" alt="GESTION COMERCIAL" border="0" /></a><a href="menu.php"><img src="images/identico.gif" style="cursor:hand;" alt="MENU PRINCIPAL" border="0"/></a><a href="cotizacion_general_menu.php"><img src="images/salir.gif" style="cursor:hand;" alt="SALIR" onclick="window.close() "/></a></td>
  </tr>
</table>
</div>
</body>
