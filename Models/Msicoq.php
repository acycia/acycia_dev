...
class oSicoq{
    ...
    public function buscarControl($tabla, $columna, $order='',$on='',$where='' ){

      try 
        {
          //echo "SELECT $columna FROM $tabla $on $where ORDER BY $order DESC LIMIT 0,1";;
     
            if($tabla!='' && $columna!='' ){ 
              
               $stm = $this->db->query("SELECT $columna FROM $tabla $on $where ORDER BY $order DESC LIMIT 0,1") ;
    ...
    public function ObtenerFecha($tabla,$columna,$id,$columna2='',$id2='')
    {
        ...
        try 
        {
            if($tabla!='' && $columna!='' && $id!='' ){ 
                ...
                $query = "SELECT * FROM $tabla WHERE $columna LIKE '%$id%'  $columna2 ";
    ...
    public function ObtenerPorMes($tabla,$columna,$id,$columna2='',$id2='')
    {
    ...
    public function Listaritems($tabla, $order='', $columna='')
    {
        ...
        try 
        {
            ...
            if($tabla!='' ){ 
                ...
                $query = "SELECT * FROM $tabla $columna ORDER BY $order ASC ";
                $stm = $this->db->query($query);
               if($stm){
                while($filas=$stm->fetch_assoc()){
                    $this->ordenc[]=$filas;
    ...
    public function ListaritemsDesc($tabla, $order='', $columna='')
    {
        ...
        try 
        {
            ...
            if($tabla!='' ){ 
                //echo "SELECT * FROM $tabla $columna ORDER BY $order ASC ";die;
                $query = "SELECT * FROM $tabla $columna ORDER BY $order DESC ";
    ...
    { 
        ...
        try 
        {
                ...
                if($consulta) 
                  $result = self::getResultados2($consulta); 
             

                if(empty($result["autorizacion"])){
                   ...
                   $stmt = $this->db->query("UPDATE $tabla SET kilospermitidosmes='". $arrayPHP['kilospermitidosmes'] ."',kilosdisponiblescompra='". $arrayPHP['kilosdisponiblescompra'] ."',totalingresados='". $arrayPHP['totalingresados'] ."',totalsalida='". $arrayPHP['totalsalida'] ."',totalinventario='". $arrayPHP['totalinventario'] ."',userfile='". $arrayPHP['userfile'] ."' WHERE autorizacion = '". $arrayPHP['autorizacion'] ."';");  
    ...
    { 
        ...
        {  
               ...
               if( !(empty($arrayPHP['autorizacion'])) )  { 
                ...
                //$arrayPHP['controladas'] = $arrayPHP['controladas']  == '1' ? 'SI' : 'NO'; 
               /*echo '<pre>';
               var_dump($arrayPHP['controladas']);
               echo '<pre>';die; */
    ...
    public function Delete($tabla,$id,$columna,$proceso,$master)
    {
      ...
      public function getResultados2($arreglo)
      {
        $rows = array();
      while($row = $arreglo->fetch_array(MYSQLI_BOTH))//MYSQLI_ASSOC array asociativo, MYSQLI_NUM array numérico
      {
