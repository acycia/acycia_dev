...
class oComprasFA{
    ...
    { 
        ...
        try 
        {
                ...
                $consulta=$this->db->query("SELECT * FROM $tabla WHERE ".$filtro." ='$id' AND proceso='ENTRADA FACTURA' ");
                if($consulta){
                 while($filas=$consulta->fetch_assoc()){
                    $this->existe[]=$filas;
                 }
                ...
                if(is_null($this->existe)){ 
                  $updatepro = $this->db->query("UPDATE $tabla SET pedido = '". $arrayPHP['pedido'] ."', factura = '". $arrayPHP['factura'] ."', fecha_plazo = '". $arrayPHP['fecha_plazo'] ."' WHERE proforma = '". $arrayPHP['proforma'] ."' ;" );
                  $updateprodetall = $this->db->query("UPDATE tbl_proceso_compras_detalle SET pedido = '". $arrayPHP['pedido'] ."', factura = '". $arrayPHP['factura'] ."' WHERE proforma = '". $arrayPHP['proforma'] ."' ;" ); 

                  $stmt = $this->db->query("INSERT INTO $tabla ($columna) VALUES ( '". $arrayPHP['proforma'] ."','". $arrayPHP['pedido'] ."','". $arrayPHP['factura'] ."','". $arrayPHP['proceso'] ."','". $arrayPHP['proveedor'] ."','". $arrayPHP['fecha'] ."','". $arrayPHP['bodega'] ."','". $arrayPHP['tipopedido'] ."','". $arrayPHP['tipoinsumo'] ."','". $arrayPHP['maquina'] ."','". $arrayPHP['plazo'] ."','". $arrayPHP['valorplazo'] ."','". $arrayPHP['fecha_plazo'] ."','" . $arrayPHP['adjunto'] . "','" . $arrayPHP['usuario'] . "','" . $arrayPHP['estado'] . "'  );");
                 
                }else{ 

                    $updatepro = $this->db->query("UPDATE $tabla SET proforma = '". $arrayPHP['proforma'] ."',pedido = '". $arrayPHP['pedido'] ."',factura = '". $arrayPHP['factura'] ."',proceso = '". $arrayPHP['proceso'] ."',proveedor = '". $arrayPHP['proveedor'] ."',fecha = '". $arrayPHP['fecha'] ."',bodega = '". $arrayPHP['bodega'] ."',tipopedido = '". $arrayPHP['tipopedido'] ."',tipoinsumo = '". $arrayPHP['tipoinsumo'] ."',maquina = '". $arrayPHP['maquina'] ."',plazo = '". $arrayPHP['plazo'] ."',valorplazo = '". $arrayPHP['valorplazo'] ."',fecha_plazo = '". $arrayPHP['fecha_plazo'] ."',adjunto = '". $arrayPHP['adjunto'] . "',usuario = '". $arrayPHP['usuario'] . "',estado = '". $arrayPHP['estado'] . "'  WHERE ".$filtro." = '". $id ."' AND proceso='ENTRADA FACTURA' ;" );
                }
    ...
    { 
        ...
        try 
        {
             ...
             foreach ($id as $key => $iditems) {
                  $consulta=$this->db->query("SELECT id FROM $tabla WHERE id ='$iditems' AND proceso='ENTRADA FACTURA'");
                  if($consulta){
                   while($filas=$consulta->fetch_assoc()){
                     $existe[]=$filas; 
                   }      
            ...
            for ($d=0,$e=0,$f=0,$g=0,$h=0,$i=0,$j=0,$k=0,$l=0,$n=0,$m=0;$d<count($cantidad);$d++,$e++,$f++,$g++,$h++,$i++,$j++,$k++,$l++,$n++,$m++)
            {
                if($id[$m]!='' )  { 
                   ...
                   $update = self::Update("UPDATE $tabla SET proforma='". $arrayPHP['proforma'] ."', pedido='". $arrayPHP['pedido'] . "', factura='". $arrayPHP['factura'] . "', proceso='". $arrayPHP['proceso'] . "', cantidad='". $cantidad[$d] . "', medida='". $medida[$l] . "', code='". $code[$e] . "', descripcion='". $descripcion[$f] . "', moneda='". $moneda[$g] . "', precio='". $precio[$h] . "', precio_total='". $precio_total[$i] . "', incoterm='". $incoterm[$j] . "', valoricot='". $valoricot[$k] . "', estado='". $arrayPHP['estado'] . "', bodega='". $bodega[$n] . "' WHERE id = '".$id[$m]."' AND proceso='ENTRADA FACTURA' ;");  
                }
            } 
