...
mysql_select_db($database_conexion1, $conexion1);
$query_ver_ref = sprintf("SELECT * FROM Tbl_cotiza_laminas WHERE N_cotizacion = %s", $colname_ver_ref);
...
mysql_select_db($database_conexion1, $conexion1);
$query_referencias = sprintf("SELECT * FROM  Tbl_referencia  ORDER BY cod_ref ASC", $colname_referencias);
...
<input name="registro1_ref" type="text" value="<?php echo $row_usuario['nombre_usuario']; ?>" size="27" /></td>
      ...
      <tr id="tr3">
        <td nowrap="nowrap" id="fuente2"><select name="ref" id="ref" onblur="if(form1.ref.value) { consultaref(); } else{ alert('Debe Seleccionar una REFERENCIA'); }">
          <option value="">Select</option>
          <?php
do {  
?>
          <option value="<?php echo $row_referencias['cod_ref']?>"><?php echo $row_referencias['cod_ref']?></option>
          <?php
...
?>
          ...
          <input name="cod_ref" type="text" value="<?php echo $row_ver_nref['N_referencia']+1; ?>" size="5" readonly="readonly"/> 
          - 
