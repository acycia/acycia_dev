...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
 $insertSQL = sprintf("INSERT INTO Tbl_referencia (cod_ref, version_ref, n_egp_ref, n_cotiz_ref, Str_referencia_m, Str_linc_m, estado_ref, registro1_ref, fecha_registro1_ref, registro2_ref, fecha_registro2_ref ) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {*/
  $insertSQL2  = sprintf("INSERT INTO Tbl_egp (n_egp, responsable_egp, codigo_usuario, fecha_egp, hora_egp, estado_egp, observacion5_egp, responsable_modificacion, fecha_modificacion, hora_modificacion, vendedor) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                       GetSQLValueString($_POST['cod_ref'], "int"),
                       GetSQLValueString($_POST['registro1_ref'], "text"),
...
mysql_select_db($database_conexion1, $conexion1);
$query_ver_ref = sprintf("SELECT * FROM Tbl_cotiza_materia_p WHERE N_cotizacion= %s", $colname_ver_ref);
...
<tr><td id="nombreusuario"><?php echo $row_usuario['nombre_usuario']; ?></td>
  ...
  <form method="post" name="form1" action="<?php echo $editFormAction; ?>">
    <table id="tabla2">
      ...
      <tr id="tr3">
        <td nowrap="nowrap" id="fuente2"><input name="cod_ref" type="text" value="<?php echo $row_ver_nref['cod_ref']+1; ?>" size="5" />
-
  <input name="version_ref" type="text" value="00" size="2" /></td>
      ...
      <tr>
        <td id="dato2"><input name="n_cotiz_ref" type="text" value="<?php echo $row_ver_ref['N_cotizacion']; ?>" size="5" /></td>
        <td id="dato2">&nbsp;</td>
        <td id="dato2"></td>
      </tr>
      <tr id="tr1">
...
do {  
?>
          <option value="<?php echo $row_ver_ref['Str_referencia']?>"<?php if (!(strcmp($row_ver_ref['N_cotizacion'], $_POST['N_cotizacion']))) {echo "selected=\"selected\"";} ?>><?php echo $row_ver_ref['Str_referencia']?></option>
          <?php
} while ($row_ver_ref= mysql_fetch_assoc($ver_ref));
  $rows = mysql_num_rows($ver_ref);
  if($rows > 0) {
      mysql_data_seek($ver_ref, 0);
	  $row_ver_ref= mysql_fetch_assoc($ver_ref);
  }
?>
        </select></td>
...
?>
        ...
        <td colspan="2" id="fuente1"><?php if($row_ver_ref['Str_linc']!=''){ ?>
          <a href="javascript:verFoto('archivosc/archivos_pdf_mp/<?php echo $row_ver_ref['Str_linc'] ?>','610','490')" target="_blank"><?php echo $row_ver_ref['Str_linc'] ?></a>
          <?php }else  echo "<span class='rojo'>No tiene archivos adjuntos</span>";  ?>
          <input type="hidden" name="Str_linc" id="Str_linc" value="<?php echo $row_ver_ref['Str_linc'] ?>"/></td>
        </tr>
      <tr>
        <td colspan="2" id="fuente1"><input name="registro2_ref" type="hidden" value="<?php echo $row_usuario['nombre_usuario']; ?>" />
          <input name="fecha_registro2_ref" type="hidden" value="<?php echo date("Y-m-d"); ?>" />
          <input name="tipo_usuario2" type="hidden" id="tipo_usuario2" value="<?php echo $row_usuario['tipo_usuario']; ?>" /></td>
        <td id="fuente1">&nbsp;</td>
        <td id="fuente1">&nbsp;</td>
      </tr>
      <tr id="tr1">
        <td colspan="2" id="fuente1"><input name="responsable_modificacion" type="hidden" value="<?php echo $row_usuario['nombre_usuario']; ?>" />
          ...
          <input name="tipo_usuario3" type="hidden" id="tipo_usuario3" value="<?php echo $row_usuario['tipo_usuario']; ?>" /></td>
        <td id="fuente1">&nbsp;</td>
        <td id="fuente1">&nbsp;</td>
        </tr>
      <tr>
      ...
      <tr>
        <td colspan="4" id="dato1"><textarea name="observacion5_egp" cols="78" rows="2" id="observacion5_egp"onKeyUp="conMayusculas(this)"></textarea></td>
      </tr>
      <tr>
