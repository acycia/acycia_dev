...
$colname_cotizacion_ref = "-1";
if (isset($_GET['cod_refe'])) {
  $colname_cotizacion_ref = (get_magic_quotes_gpc()) ? $_GET['cod_refe'] : addslashes($_GET['cod_refe']);
}
$colname_cotizacion_nit = "-1";
if (isset($_GET['Str_nit'])) {
  $colname_cotizacion_nit = (get_magic_quotes_gpc()) ? $_GET['Str_nit'] : addslashes($_GET['Str_nit']);
}
mysql_select_db($database_conexion1, $conexion1);
...
Tbl_cotiza_packing.N_referencia_c='%s' and Tbl_cotiza_packing.Str_nit='%s' and
Tbl_cotiza_packing.Str_nit=cliente.nit_c and
Tbl_cotiza_packing.B_generica='0' and
Tbl_cotiza_packing.B_estado<>'2'", $colname_ver_nueva,$colname_cotizacion_ref,$colname_cotizacion_nit);//desaparece las rechazadas de la vista
...
Tbl_cotiza_packing.Str_nit=cliente.nit_c and
Tbl_cotiza_packing.B_generica='1' ORDER BY Tbl_cotiza_packing.fecha_creacion DESC", $colname_ver_existente,$colname_cotizacion_ref,$colname_cotizacion_nit);
...
{ ?>
      <table id="tablainterna" >
        ...
        <tr>
          <td id="subppal4">PRECIO UNIDAD($)</td>
          <?php  for ($j=0;$j<=$num1-1;$j++) { ?>
          <td id="fuente2">
        ...
        <tr>
          ...
          <?php  for ($j=0;$j<=$num1-1;$j++) { ?>
          <td id="fuente2"><?php $var=mysql_result($ver_nueva,$j,B_estado); if($var=='1'){ echo "ACEPTADA"; }else if($var=='0'){echo "PENDIENTE";}else if($var=='2'){echo "RECHAZADA";} ?></td>
          <?php } ?>
        </tr>                       
      </table>
      <?php }?></td>
...
{ ?>
      <table id="tablainterna" >
        ...
        <tr>
          ...
          <td id="subppal4">ESTADO</td>
          <?php  for ($i=0;$i<=$num2-1;$i++) { ?>
          <td id="fuente2"><?php $var=mysql_result($ver_existente,$i,B_estado); if($var=='1'){ echo "ACEPTADA"; }else if($var=='0'){echo "PENDIENTE";}else if($var=='2'){echo "RECHAZADA";} ?></td>
          <?php } ?>
        </tr>                        
      </table>
      <?php }?></td>
