...
{
/*SELECT * FROM Tbl_reg_produccion WHERE DATE(fecha_ini_rp) BETWEEN '$fecha1' AND '$fecha2' and
DATE(fecha_fin_rp) BETWEEN '$fecha1' AND '$fecha2' GROUP BY id_op_rp DESC*/
...
if($fecha1 != '' && $fecha2 != '' && $ref !='' &&  $mp =='' )
{
  $query_consumo = "SELECT * FROM Tbl_orden_produccion,Tbl_reg_kilo_producido WHERE Tbl_orden_produccion.id_op=Tbl_reg_kilo_producido.op_rp AND Tbl_orden_produccion.id_ref_op='$ref' AND Tbl_orden_produccion.b_estado_op > 0 AND DATE(Tbl_reg_kilo_producido.fecha_rkp) BETWEEN '$fecha1' AND '$fecha2' and Tbl_reg_kilo_producido.id_proceso_rkp='$proceso' ORDER BY Tbl_orden_produccion.id_op DESC";
}
//Filtra fechas y materia prima
if($fecha1 != '' && $fecha2 != '' && $ref =='' &&  $mp !='' )
{
mysql_select_db($database_conexion1, $conexion1);
$query_consumo = "SELECT * FROM Tbl_orden_produccion,Tbl_reg_kilo_producido WHERE Tbl_orden_produccion.id_op=Tbl_reg_kilo_producido.op_rp AND Tbl_reg_kilo_producido.id_rpp_rp='$mp' AND Tbl_orden_produccion.b_estado_op > 0 AND DATE(Tbl_reg_kilo_producido.fecha_rkp) BETWEEN '$fecha1' AND '$fecha2' and Tbl_reg_kilo_producido.id_proceso_rkp='$proceso' ORDER BY Tbl_orden_produccion.id_op DESC";
}
//Filtra todos llenos
if($fecha1 != '' && $fecha2 != '' && $ref !='' &&  $mp !='' )
{
  $query_consumo = "SELECT * FROM Tbl_orden_produccion,Tbl_reg_kilo_producido WHERE Tbl_orden_produccion.id_op=Tbl_reg_kilo_producido.op_rp AND Tbl_orden_produccion.id_ref_op='$ref' AND Tbl_reg_kilo_producido.id_rpp_rp='$mp' AND Tbl_orden_produccion.b_estado_op > 0 AND DATE(Tbl_reg_kilo_producido.fecha_rkp) BETWEEN '$fecha1' AND '$fecha2' and Tbl_reg_kilo_producido.id_proceso_rkp='$proceso' ORDER BY Tbl_orden_produccion.id_op DESC";
}
...
<td colspan="4" nowrap="nowrap" id="titulo2"> CONSUMO DE MATERIAS PRIMAS IMPRESION</td>
...
<tr>
    ...
    <td colspan="5" id="dato3"><a href="produccion_registro_extrusion_listado.php"><img src="images/e.gif" alt="LISTADO EXTRUSION"title="LISTADO EXTRUSION" border="0" style="cursor:hand;" /></a><a href="produccion_registro_impresion_listado.php"><img src="images/i.gif" alt="LISTADO IMPRESION"title="LISTADO IMPRESION" border="0" style="cursor:hand;" /></a><a href="consumo_tiempos_imp.php"><img src="images/rt.gif" alt="LISTADO DE TIEMPOS"title="LISTADO DE TIEMPOS" border="0" style="cursor:hand;" /></a><a href="javascript:location.reload()"><img src="images/ciclo1.gif" alt="RESTAURAR" title="RESTAURAR" border="0" style="cursor:hand;"/></a>
      <input type="button" value="Descarga Excel" onclick="window.location = 'consumo_materias_primas_excel_imp.php?fechaI=<?php echo $_GET['fechaI']?>&fechaF=<?php echo $_GET['fechaF'] ?>&ref=<?php echo $_GET['ref']?>&mp=<?php echo $_GET['mp'] ?>'" / /></td>
    </tr>
  </table>
  </form>
  <table id="tabla1">    
