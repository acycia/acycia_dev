...
mysql_select_db($database_conexion1, $conexion1);
$query_costos = "SELECT * FROM Tbl_orden_produccion WHERE b_estado_op >= $proceso AND DATE(f_impresion) BETWEEN '$fecha1' AND '$fecha2' ORDER BY id_op DESC";
...
<form action="consumo_tiempos_imp2.php" method="get" name="form1">
<table id="tabla1">
<tr>
<td colspan="3" owrap="nowrap" id="codigo" >CODIGO : R1 - F03</td>
<td colspan="3" nowrap="nowrap" id="titulo2" >TIEMPOS DE IMPRESION</td>
...
<tr>
  <td colspan="8" id="fuente2">FECHA INICIO:
    <input name="fecha_ini" type="date" required="required" id="fecha_ini" min="2000-01-02" size="10" value="<?php echo date('Y-m-01');?>"/>
FECHA FIN:
<input name="fecha_fin" type="date" id="fecha_fin" min="2000-01-02" size="10" required="required" value="<?php echo fecha();?>"/>
<!--onChange="if(form1.fecha_ini.value && form1.fecha_fin.value) { consulta_ref_xproceso_tiempos(); }else { alert('Debe seleccionar las dos fechas')}"-->
REF:
...
do {  
?>
  <option value="<?php echo $row_orden_produccion['cod_ref']?>"><?php echo $row_orden_produccion['cod_ref']?></option>
  <?php
...
<input type="submit" name="submit" value="FILTRO"/></td>
  ...
  <tr>
    ...
    <td colspan="3" id="dato3"><a href="produccion_registro_extrusion_listado.php"><img src="images/e.gif" alt="LISTADO EXTRUSION"title="LISTADO EXTRUSION" border="0" style="cursor:hand;" /></a><a href="produccion_registro_impresion_listado.php"><img src="images/i.gif" alt="LISTADO IMPRESION"title="LISTADO IMPRESION" border="0" style="cursor:hand;" /></a><a href="produccion_registro_sellado_listado.php"><img src="images/s.gif" alt="LISTADO SELLADO"title="LISTADO SELLADO" border="0" style="cursor:hand;" /></a><a href="consumo_materias_primas_imp.php"><img src="images/mp.gif" alt="LISTADO DE MATERIAS PRIMAS"title="LISTADO DE MATERIAS PRIMAS" border="0" style="cursor:hand;" /></a><a href="costos_listado_ref_xproceso.php"><img src="images/rp.gif" alt="LISTADO REF KILOS X PROCESO"title="LISTADO REF KILOS X PROCESO" border="0" style="cursor:hand;" /></a><a href="javascript:location.reload()"><img src="images/ciclo1.gif" alt="RESTAURAR" title="RESTAURAR" border="0" style="cursor:hand;"/></a></td>        
    <td colspan="2" id="dato3"><input type="button" value="Descarga Excel" onclick="window.location = 'consumo_tiempos_imp_excel.php?fecha_ini=<?php echo $fecha1 ?>&amp;fecha_fin=<?php echo $fecha2 ?>&amp;id_op=<?php echo $_GET['id_op']?>'" /></td>
    </tr>
    </table>  
  <table id="tabla1" >    
    <tr id="tr1">
  ...
  <tr onMouseOver="uno(this,'CBCBE4');" onMouseOut="dos(this,'#FFFFFF');" bgcolor="#FFFFFF">
    <td id="dato1"><?php echo $row_costos['id_op'];?></td>
