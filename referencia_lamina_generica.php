...
GetSQLValueString($_POST['cod_gen'], "int"),
GetSQLValueString($_POST['n_cotiz_ref'], "int"),
...
$updateGoTo = "referencia_lamina_vista.php?cod_ref=" . $_POST['cod_gen']  . "&tipo=" . $_POST['tipo_usuario'] ."";
  if (isset($_SERVER['QUERY_STRING'])) {
    $updateGoTo .= (strpos($updateGoTo, '?')) ? "&" : "?";
    $updateGoTo .= $_SERVER['QUERY_STRING'];
  }
 ...
 ?>	<!--<SCRIPT>
  window.alert("Cotizacion guardada con exito");
  window.location =("cotizaciones_clientes.php");
</SCRIPT>--><?php
}
...
//IMPRIME TODO LO DEL EGP
$colname_ver_egp = "-1";
if (isset($_GET['id_ref'])) {
  $colname_ver_egp = (get_magic_quotes_gpc()) ? $_GET['id_ref'] : addslashes($_GET['id_ref']);
}
mysql_select_db($database_conexion1, $conexion1);
$query_ver_egp = sprintf("SELECT * FROM  Tbl_cotizaciones,Tbl_referencia,Tbl_egp  WHERE Tbl_referencia.id_ref='%s' AND Tbl_referencia.cod_ref=Tbl_egp.n_egp  and Tbl_referencia.cod_ref=Tbl_cotizaciones.N_cotizacion AND Tbl_cotizaciones.Str_tipo='LAMINA' AND  Tbl_referencia.B_generica='1' ORDER BY cod_ref ASC" , $colname_ver_egp);
...
mysql_select_db($database_conexion1, $conexion1);
$query_referencias = sprintf("SELECT * FROM  Tbl_referencia,Tbl_cotizaciones  WHERE Tbl_referencia.B_generica='1' and Tbl_cotizaciones.N_cotizacion=Tbl_referencia.n_cotiz_ref and Tbl_cotizaciones.Str_tipo='LAMINA' ORDER BY cod_ref ASC ", $colname_referencias);
...
<tr><td id="nombreusuario"><?php echo $row_usuario['nombre_usuario']; ?></td>
  ...
  <form action="<?php echo $editFormAction; ?>" method="post" name="form1" onSubmit="MM_validateForm('ancho_ref','','RisNum','N_repeticion_ref','','RisNum','calibre_ref','','RisNum','N_diametro_max_l','','RisNum','N_peso_max_l','','RisNum','N_cantidad_metros_r_l','','RisNum','peso_ref','','RisNum','N_embobinado','','R','cantidad_egp','','RisNum','B_generica','','RisNum');return document.MM_returnValue">
    <table id="tabla2">
      <tr id="tr1">
...
<input name="registro1_ref" type="text" value="<?php echo $row_usuario['nombre_usuario']; ?>" size="27" /></td>
      ...
      <tr id="tr3">
        ...
        <td width="127" nowrap="nowrap" id="fuente2">Estado</td>
        <td width="234" id="fuente2">Arte</td>
      </tr>
      <tr id="tr3">
        <td nowrap="nowrap" id="fuente2"><select name="ref" id="ref" onBlur="if(form1.ref.value) { genericalaminaref(); } else{ alert('Debe Seleccionar una REFERENCIA'); }">
...
?>
        ...
        <td id="fuente2"><select name="estado_ref" id="estado_ref">
          <option value="1" <?php if (!(strcmp(1, $row_ver_egp['estado_egp']))) {echo "selected=\"selected\"";} ?>>Activa</option>
          <option value="0" <?php if (!(strcmp(0, $row_ver_egp['estado_egp']))) {echo "selected=\"selected\"";} ?>>Inactiva</option>
        </select></td>
      ...
      <tr>
        <td id="dato2"><input name="n_cotiz_ref" type="text" value="<?php echo $_GET['n_cotiz_ref'];//$row_consecutivo['N_cotizacion']; ?>" size="5" readonly/></td>
      ...
      <tr>
        ...
        <td id="dato1"><input name="N_repeticion_ref" type="text" id="N_repeticion_ref"  value="<?php echo $row_referencia_editar['N_repeticion_l']; ?>" size="10"/></td>
        <td id="dato1"><input name="calibre_ref" type="text" id="calibre_ref" value="<?php echo $row_referencia_editar['calibre_ref']; ?>" size="10"/></td>
        <td id="dato1"><input name="N_diametro_max_l" type="text" id="N_diametro_max_l" size="10" value="<?php echo $row_referencia_editar['N_diametro_max_l']?>"/></td>
      </tr>
      <tr id="tr1">
      ...
      <tr>
        <td id="dato1"><input name="N_peso_max_l" type="text" id="N_peso_max_l" size="10" value="<?php echo $row_referencia_editar['N_peso_max_l'] ?>"/></td>
        <td id="dato1"><input name="N_cantidad_metros_r_l" type="text" id="N_cantidad_metros_r_l" size="10" value="<?php echo $row_referencia_editar['N_cantidad_metros_r_l']?>"/></td>
        <td id="dato1"><?php		
        $var1=$row_referencia_editar['N_ancho'];
        $var2=$row_referencia_editar['N_calibre'];
        $var3=($var1);
        $var4=($var2);
		$peso=(24*($var4));	
	    $ps=($peso*100)/100;
        $psm= number_format($ps,2);
        ?>          <input name="peso_ref" type="text" id="peso_ref" value="<?php echo $psm; ?>" size="10" maxlength="8" readonly/></td>
        <td id="dato1"><input type="text" name="N_embobinado" id="N_embobinado"value="<?php echo $row_referencia_editar['N_embobinado_l'] ?>" size="2" maxlength="2"onkeyup="conMayusculas(this)"/>          <a href="javascript:verFoto('embobinado_lamina.php','575','510')" >Ver Cuadro</a></td>
      </tr>
      <tr id="tr1">
      ...
      <tr>
        ...
        <input name="cantidad_egp" type="text" id="cantidad_egp" value="<?php echo $row_referencia_editar['N_cantidad_metros_r_l']; ?>" size="10"/></td>
        </tr>
      <tr id="tr1">
...
Incluir Fecha de Caducidad </td>
      ...
      <tr>
        <td id="fuente6"><input type="text" name="disenador_egp" value="<?php echo $row_ver_egp['disenador_egp']; ?>" size="20" onKeyUp="conMayusculas(this)"/></td>
        <td id="fuente6"><input type="text" name="telef_disenador_egp" value="<?php echo $row_ver_egp['telef_disenador_egp']; ?>" size="20" onKeyUp="return ValNumero(this)"/></td>
        <td id="fuente6">&nbsp;</td>
      ...
      <tr id="tr1">
        <td colspan="2" id="fuente1"><input name="cod_gen" type="hidden" value="<?php echo $row_referencia_editar['cod_ref']; ?>" />
    ...
    <input type="hidden" name="Str_nit" value="<?php echo $_GET['Str_nit']; ?>"> 
