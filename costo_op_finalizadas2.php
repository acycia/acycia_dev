...
//Filtra todos vacios
if($id_op== '0' && $id_ref == '0' && $estado == '')
{
$query_fichas_tecnicas = "SELECT * FROM Tbl_orden_produccion WHERE b_estado_op > 0 ORDER BY id_op DESC";
}
if($id_op!= '0' && $id_ref == '0' && $estado == '')
{
$query_fichas_tecnicas = "SELECT * FROM Tbl_orden_produccion WHERE id_op='$id_op' AND  b_estado_op > 0 ";
}
if($id_op== '0' && $id_ref != '0' && $estado == '')
{
$query_fichas_tecnicas = "SELECT * FROM Tbl_orden_produccion WHERE id_ref_op='$id_ref' AND  b_estado_op > 0 ";
}
if($id_op== '0' && $id_ref == '0' && $estado != '')
{
$query_fichas_tecnicas = "SELECT * FROM Tbl_orden_produccion WHERE b_estado_op='$estado' AND  b_estado_op > 0 ";
}
if($id_op!= '0' && $id_ref == '0' && $estado != '')
{
$query_fichas_tecnicas = "SELECT * FROM Tbl_orden_produccion WHERE id_op='$id_op' AND b_estado_op='$estado'";
}
if($id_op== '0' && $id_ref != '0' && $estado != '')
{
$query_fichas_tecnicas = "SELECT * FROM Tbl_orden_produccion WHERE id_ref_op='$id_ref' AND b_estado_op='$estado'";
}
if($id_op!= '0' && $id_ref != '0' && $estado != '')
{
$query_fichas_tecnicas = "SELECT * FROM Tbl_orden_produccion WHERE id_op='$id_op' AND id_ref_op='$id_ref' AND b_estado_op='$estado'";
}
if($id_op== '0' && $id_ref == '0' && $estado == '' && $anos != '0' && $fecha != '0')
{
$query_fichas_tecnicas = "SELECT DISTINCT Tbl_reg_produccion.id_op_rp AS id_op,Tbl_orden_produccion.int_cod_ref_op,Tbl_orden_produccion.str_tipo_bolsa_op,Tbl_orden_produccion.b_estado_op FROM Tbl_orden_produccion,Tbl_reg_produccion WHERE Tbl_orden_produccion.id_op=Tbl_reg_produccion.id_op_rp AND fecha_ini_rp  BETWEEN DATE_FORMAT('$fecha', '%Y-%m-01') AND DATE_FORMAT('$fecha', '%Y-%m-31') AND fecha_fin_rp  BETWEEN DATE_FORMAT('$fecha', '%Y-%m-01') AND DATE_FORMAT('$fecha', '%Y-%m-31') ORDER BY Tbl_reg_produccion.id_op_rp DESC";
}
...
do {  
?>
        <option value="<?php echo $row_lista['id_op']?>"<?php if (!(strcmp($row_lista['id_op'], $_GET['id_op']))) {echo "selected=\"selected\"";} ?>><?php echo $row_lista['id_op']?></option>
        <?php
...
do {  
?>
        <option value="<?php echo $row_referencias['id_ref']?>"<?php if (!(strcmp($row_lista['id_op'], $_GET['id_ref']))) {echo "selected=\"selected\"";} ?>><?php echo $row_referencias['cod_ref']?></option>
        <?php
...
?>
      ...
      <select name="estado" id="estado" style="width:100px" >
       ...
       <!-- <option value="0"<?php if (!(strcmp("0", $_GET['estado']))) {echo "selected=\"selected\"";} ?>>INGRESADA</option>-->
        <option value="1"<?php if (!(strcmp("1", $_GET['estado']))) {echo "selected=\"selected\"";} ?>>EXTRUSION</option>
        <option value="2"<?php if (!(strcmp("2", $_GET['estado']))) {echo "selected=\"selected\"";} ?>>IMPRESION</option>
        <option value="3"<?php if (!(strcmp("3", $_GET['estado']))) {echo "selected=\"selected\"";} ?>>REFILADO</option>
        <option value="4"<?php if (!(strcmp("4", $_GET['estado']))) {echo "selected=\"selected\"";} ?>>SELLADO</option>
        <option value="5"<?php if (!(strcmp("5", $_GET['estado']))) {echo "selected=\"selected\"";} ?>>FINALIZADA</option>
       </select>
 ...
 <table id="tabla1">
  ...
  <?php do { ?>
    <tr onMouseOver="uno(this,'CBCBE4');" onMouseOut="dos(this,'#FFFFFF');" bgcolor="#FFFFFF">
      ...
      <td id="dato1" nowrap><a href="costos_op_add.php?id_op=<?php echo $row_consumo['id_op']; ?>" target="_top" style="text-decoration:none; color:#000000"><?php echo $row_consumo['str_tipo_bolsa_op'];?></a></td>
	...
	$manoObra=($horas_imp*($valorHoraImp+$valorHoraImpDemas))/$KILOSREALESIMP;//$ costo de 1 kilo mano obra en 1 hora
	 ...
	 //}
      ...
      <td id="dato3"><a href="costos_op_add.php?id_op=<?php echo $row_consumo['id_op']; ?>" target="_top" style="text-decoration:none; color:#000000"> <?php //echo $KILOSREALESSELL =($KILOSREALESIMP-$kilos_despimp);
       ...
       ?></a></td>
      <td id="dato3">
        <a href="costos_op_add.php?id_op=<?php echo $row_consumo['id_op']; ?>" target="_top" style="text-decoration:none; color:#000000"> 
        <?php 
	  	 //MATERIA PRIMA
	   	 mysql_select_db($database_conexion1, $conexion1);
