...
GetSQLValueString($_POST['cod_gen'], "text"),
GetSQLValueString($_POST['n_cotiz_ref'], "int"),
...
$insertSQL6 = sprintf("UPDATE Tbl_cotiza_bolsa SET N_referencia_c=%s WHERE N_cotizacion='$cotiz' and N_referencia_c='$cod_ref' and Str_nit='$Str_nit'",
GetSQLValueString($cod_ref, "int"),
GetSQLValueString($cotiz, "text"),
GetSQLValueString($cod_ref, "int"),
GetSQLValueString($Str_nit, "int"));
mysql_select_db($database_conexion1, $conexion1);
$Result6 = mysql_query($insertSQL6, $conexion1) or die(mysql_error());*/


$updateGoTo = "referencia_bolsa_vista.php?cod_ref=" . $_POST['cod_gen']  . "&tipo=" . $_POST['tipo_usuario'] ."";
  if (isset($_SERVER['QUERY_STRING'])) {
    $updateGoTo .= (strpos($updateGoTo, '?')) ? "&" : "?";
    $updateGoTo .= $_SERVER['QUERY_STRING'];
  }
  header(sprintf("Location: %s", $updateGoTo));
  /*echo "<script type=\"text/javascript\">alert(\"Cotizacion guardada con exito\");</script>";
  */
/*   <!--<script language="javascript">
      document.write('Cotizacion guardada con exito');
    </script>
	<SCRIPT>
  window.alert("Cotizacion guardada con exito");
  window.location ='cotizacion_general_laminas_generica.php
</SCRIPT>-->
<script language="Javascript" type="text/javascript">
  function confirmar()
{
	return confirm	("Cotizacion guardada con exito");
	}
</script>
<a href="http://www.acycia.com/intranet/referencia_bolsa_generica2.php" onclick="return confirmar()">volver</a>-->*/
 ?>	<!--<SCRIPT>
  window.alert("Cotizacion guardada con exito");
...
//EXPLODE PARA DIVIDIR LA VARIABLE REF QUE VIENE LA REF Y EL COD DE VISUALIZCION CON BOTON REF. 
/*$idycod=$_GET['ref'];
$datos = explode( ',',$idycod);
$id_ref=$datos[0];
$cod_ref=$datos[1];*/
//fin
$colname_usuario = "-1";
...
mysql_select_db($database_conexion1, $conexion1);
$query_ver_egp = sprintf("SELECT * FROM  Tbl_cotizaciones,Tbl_referencia,Tbl_egp  WHERE Tbl_referencia.id_ref='%s' AND Tbl_referencia.cod_ref=Tbl_egp.n_egp  and Tbl_referencia.cod_ref=Tbl_cotizaciones.N_cotizacion AND Tbl_cotizaciones.Str_tipo='BOLSA' AND  Tbl_referencia.B_generica='1' ORDER BY cod_ref ASC" , $colname_ver_egp);
...
<tr><td id="nombreusuario"><?php echo $row_usuario['nombre_usuario']; ?></td>
  ...
  <form action="<?php echo $editFormAction; ?>" method="post" enctype="multipart/form-data" name="form1" onSubmit="MM_validateForm('ancho_ref','','RisNum','largo_ref','','RisNum','solapa_ref','','RisNum','bolsillo_guia_ref','','RisNum','calibre_ref','','RisNum','peso_millar_ref','','RisNum','B_fuelle','','RisNum','B_generica','','RisNum');return document.MM_returnValue">
    <table id="tabla2">
      ...
      <tr>
        <td width="135" rowspan="6" id="dato2"><img src="images/logoacyc.jpg"/></td>
      ...
      <tr id="tr1">
        <td width="184" nowrap="nowrap" id="dato2">Fecha Ingreso 
...
<input name="registro1_ref" type="text" value="<?php echo $row_usuario['nombre_usuario']; ?>" size="27" /></td>
      ...
      <tr id="tr3">
        ...
        <td width="125" nowrap="nowrap" id="fuente2">Estado</td>
        <td width="236" id="fuente2">Arte</td>
      </tr>
      <tr id="tr3">
        <td nowrap="nowrap" id="fuente2">
<select name="ref" id="ref" onBlur="if(form1.ref.value) { consultagenerica2(); } else{ alert('Debe Seleccionar una REFERENCIA'); }">
...
<input name="version_ref" type="text" value="<?php echo $row_referencia_editar['version_ref']; ?>" size="2" /></td>
      ...
      <tr>
        ...
        <td id="dato1"><input name="peso_millar_ref" type="text" id="peso_millar_ref" value="<?php echo $row_referencia_editar['peso_millar_ref']; ?>" size="10" readonly/></td>
...
<td nowrap="nowrap" id="detalle1"><input <?php if (!(strcmp($row_ver_egp['arte_sum_egp'],1))) {echo "checked=\"checked\"";} ?> name="arte_sum_egp" type="checkbox" value="1" />
      ...
      <tr id="tr1">
        <td colspan="2" id="fuente1">Ultima Modificaci&oacute;n : 
          <input name="cod_gen" type="hidden" value="<?php echo $row_referencia_editar['cod_ref']; ?>" />
