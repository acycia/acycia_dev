...
if($fecha1 != '' && $fecha2 != '')
{
$query_costos = "SELECT * FROM TblProcesoEmpleado WHERE DATE(fechainicial_empleado) BETWEEN '$fecha1'
...
<div class="spiffy_content">
        ...
        <table id="tabla1">
          <tr>
            ...
            <td colspan="2" id="fuente3">
              <input type="button" value="Excel Detalle" onclick="window.location = 'costos_liquidacion_mano_obra_excel.php?fecha_ini=<?php echo $fecha1 ?>&amp;fecha_fin=<?php echo $fecha2 ?>'" / />
              </td>
            <td colspan="2" id="fuente3"><a href="proceso_empleado_add.php"> <img src="images/mas.gif" alt="ADD EMPLEADO" title="ADD EMPLEADO" border="0" style="cursor:hand;" /></a><a href="tipos_procesos.php"><img src="images/p.gif" title="TIPOS DE PROCESOS" alt="TIPOS DE PROCESOS" border="0" style="cursor:hand;" /></a><a href="javascript:location.reload()"><img src="images/ciclo1.gif" alt="CARGAR LISTADO" title="CARGAR LISTADO" border="0" style="cursor:hand;"/></a><a href="turnos.php"><img src="images/t.gif" style="cursor:hand;" alt="TURNOS" title="TURNOS" border="0"/></a></td>
            </tr>
          <tr id="tr1">
            <td id="titulo4" nowrap>NOMBRE</td>
            <td id="titulo4" nowrap>CARGO</td>
            <td id="titulo4" nowrap>DIAS LABORADOS</td>
            <td id="titulo4" nowrap>Total Diciembre</td>
            <td id="titulo4" nowrap>Costo Hora</td>
            <td id="titulo4" nowrap>Costo Hora Media</td>
          ...
          <?php do { ?>
            <tr onMouseOver="uno(this,'CBCBE4');" onMouseOut="dos(this,'#FFFFFF');" bgcolor="#FFFFFF">
              <td id="dato1" nowrap><a href="proceso_empleado_edit.php?id_pem=<?php echo $row_costos['id_pem']; ?>" target="_top" style="text-decoration:none; color:#000000">
                <?php $codigo_empleado=$row_costos['codigo_empleado']; 	
	$sqlnom="SELECT nombre_empleado, apellido_empleado FROM empleado WHERE codigo_empleado=$codigo_empleado";
	$resultnom=mysql_query($sqlnom); $numnom=mysql_num_rows($resultnom);
	if ($numnom>='1') { $nombre_empleado=mysql_result($resultnom,0,'nombre_empleado'); $apellido_empleado=mysql_result($resultnom,0,'apellido_empleado'); 
	...
	echo $proceso_empleado; }?>
              ...
              <td id="dato2"><a href="proceso_empleado_edit.php?id_pem=<?php echo $row_costos['id_pem']; ?>" target="_top" style="text-decoration:none; color:#000000"><?php $costoEmp= $row_costos['costo_empleado'];echo numeros_format($costoEmp) ?></a></td>
              <td id="dato2"><a href="proceso_empleado_edit.php?id_pem=<?php echo $row_costos['id_pem']; ?>" target="_top" style="text-decoration:none; color:#000000"><?php $costoHora=$row_costos['costo_empleado']/$row_costos['dias_empleado'];echo numeros_format($costoHora) ?></a></td>
              <td id="dato2"><a href="proceso_empleado_edit.php?id_pem=<?php echo $row_costos['id_pem']; ?>" target="_top" style="text-decoration:none; color:#000000">
                <?php $p_empleado=$row_costos['proceso_empleado']; 	
	$sqlchm="SELECT SUM(dias_empleado) AS dias, SUM(costo_empleado) AS costo FROM TblProcesoEmpleado WHERE proceso_empleado=$p_empleado ";
	$resultchm=mysql_query($sqlchm); $numchm=mysql_num_rows($resultchm);
	if ($numchm>='1') { 
	$cost=mysql_result($resultchm,0,'costo'); 
	$dias=mysql_result($resultchm,0,'dias'); 
	$costoHoraM=$cost/$dias;
	echo numeros_format($costoHoraM); }
	?>
                </a></td>    
              <td id="dato2" ><a href="proceso_empleado_edit.php?id_pem=<?php echo $row_costos['id_pem']; ?>" target="_top" style="text-decoration:none; color:#000000">
	...
	echo $cant_empleado; }
	?> </a></td>
              <td id="dato2"><a href="proceso_empleado_edit.php?id_pem=<?php echo $row_costos['id_pem']; ?>" target="_top" style="text-decoration:none; color:#000000"><?php $operarioxMaqu=$cant_empleado/$Tmaquinas;echo $operarioxMaqu;?></a></td> 
              <td id="dato2"><a href="proceso_empleado_edit.php?id_pem=<?php echo $row_costos['id_pem']; ?>" target="_top" style="text-decoration:none; color:#000000">
                <?php 
	//foreach ()
	
	$operarioxTurnoM=($operarioxMaqu/'3,96');
	
	
	
	echo $operarioxTurnoM; ?></a></td> 
              <td id="dato2"><a href="proceso_empleado_edit.php?id_pem=<?php echo $row_costos['id_pem']; ?>" target="_top" style="text-decoration:none; color:#000000"><?php $costoxPxLxT=($costoHoraM*$operarioxTurnoM);echo "$  ";echo numeros_format($operarioxTurnoM); ?></a></td> 
              </tr>
