...
if($ref_ac != '0'&&$id_c == '0')
{
  $query_refcliente = "SELECT * FROM Tbl_refcliente WHERE int_ref_ac_rc='$ref_ac' ORDER BY int_ref_ac_rc DESC";
}
//Filtra id_c lleno
if($ref_ac == '0'&&$id_c != '0')
{
  $query_refcliente = "SELECT * FROM Tbl_refcliente WHERE id_c_rc='$id_c' ORDER BY id_c_rc DESC";
}
//Filtra todos lleno
if($ref_ac != '0'&&$id_c != '0')
{
  $query_refcliente = "SELECT * FROM Tbl_refcliente WHERE id_c_rc='$id_c' AND int_ref_ac_rc='$ref_ac' ORDER BY id_c_rc DESC";
}
    ...
    <table class="table table-bordered table-sm">
      ...
      <tr>
        <td colspan="3" id="fuente2"><select class="busqueda selectsMini" name="ref_ac" id="ref_ac">
          <option value="0" <?php if (!(strcmp(0, $_GET['ref_ac']))) {echo "selected=\"selected\"";} ?>>Seleccione la Referencia</option>
          <?php
          do {  
            ?>
            <option value="<?php echo $row_numero['int_ref_ac_rc']?>"<?php if (!(strcmp($row_numero['int_ref_ac_rc'], $_GET['ref_ac']))) {echo "selected=\"selected\"";} ?>><?php echo $row_numero['int_ref_ac_rc']?></option>
            <?php
        ...
        <select class="busqueda selectsMini" name="id_c" id="id_c"style="width:350px">
          ...
          do {  
            ?>
            <option value="<?php echo $row_cliente['id_c']?>"<?php if (!(strcmp($row_cliente['id_c'], $_GET['id_c']))) {echo "selected=\"selected\"";} ?>>
          ...
          ?>
        </select>    <input type="submit" name="Submit" value="FILTRO" onclick="if(consulta.ref_ac.value=='0' && consulta.id_c.value=='0' && consulta.fecha.value=='0') { alert('DEBE SELECCIONAR UNA OPCION'); }"/></td>
      </tr>
    </table>
  </form>
