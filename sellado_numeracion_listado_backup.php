...
mysql_select_db($database_conexion1, $conexion1);
$query_lista = "SELECT int_op_n FROM Tbl_numeracion WHERE fecha_ingreso_n <= '2016-12-31' AND existeTiq_n='1' ORDER BY int_op_n DESC";
  ...
  <?php do { ?>
    <tr onMouseOver="uno(this,'CBCBE4');" onMouseOut="dos(this,'#FFFFFF');" bgcolor="#FFFFFF">
      <?php 
        ...
        $sqlp="SELECT int_op_tn,int_paquete_tn,int_caja_tn,int_undxcaja_tn,int_undxpaq_tn,fecha_ingreso_tn FROM Tbl_tiquete_numeracion WHERE int_op_tn='$id_op' ORDER BY  int_caja_tn DESC LIMIT 1"; 
        $resultp=mysql_query($sqlp); 
        $nump=mysql_num_rows($resultp); 
        if($nump >= '1') 
        { $paquete_tn=mysql_result($resultp,0,'int_paquete_tn');
        $caja_tn=mysql_result($resultp,0,'int_caja_tn'); 
		$undxca=mysql_result($resultp,0,'int_undxcaja_tn'); 
		$undxpaq=mysql_result($resultp,0,'int_undxpaq_tn'); 
		$fecha=mysql_result($resultp,0,'fecha_ingreso_tn'); 
		}else { echo "";}*/ ?>
      <td id="dato2"><strong><?php echo $row_numeracion['int_op_n']; ?></strong></td>
  <!--    <td nowrap="nowrap" id="dato2"><a href="sellado_control_numeracion_edit.php?id_op=<?php echo $row_numeracion['int_op_n'];?>&int_caja_n=<?php echo $row_numeracion['int_caja_n'];?>" target="_parent" style="text-decoration:none; color:#000000"><?php echo $row_numeracion['int_paquete_tn'];?></td> -->     
