...
if($id_pm== '0' && $fecha == '0')
{
  $query_mezclas = "SELECT * FROM Tbl_produccion_mezclas ORDER BY Tbl_produccion_mezclas.int_cod_ref_pm DESC";
}
//Filtra Tbl_mezclas lleno
if($id_pm != '0' && $fecha == '0')
{
  $query_mezclas = "SELECT * FROM Tbl_produccion_mezclas WHERE Tbl_produccion_mezclas.id_pm =$id_pm ORDER BY Tbl_produccion_mezclas.int_cod_ref_pm DESC";
}
//Filtra fecha lleno
if($fecha != '0'  && $id_pm == '0'  )
{
  ...
  $fecha2 = $fecha + 1;
  $fecha2 = "$fecha2-01-01";
  $query_mezclas = "SELECT * FROM Tbl_produccion_mezclas WHERE Tbl_produccion_mezclas.fecha_registro_pm BETWEEN '$fecha1' AND '$fecha2' ORDER BY Tbl_produccion_mezclas.int_cod_ref_pm DESC";
}
...
if($id_pm != '0' && $fecha != '0')
{
  ...
  $fecha2 = $fecha + 1;
  $fecha2 = "$fecha2-01-01";
  $query_mezclas = "SELECT * FROM Tbl_produccion_mezclas WHERE  fecha_registro_pm BETWEEN '$fecha1' AND '$fecha2' ORDER BY Tbl_produccion_mezclas.int_cod_ref_pm DESC";
}
  ...
  <form action="produccion_mezclas2.php" method="get" name="consulta">
   ...
   <tr>
     <td id="fuente2">
       ...
       <option value="0"<?php if (!(strcmp(0, $id_pm))) {echo "selected=\"selected\"";} ?>>REF.</option>
         <?php  foreach($row_id_pm as $row_id_pm ) { ?>
          <option value="<?php echo $row_id_pm['id_pm']?>"<?php if (!(strcmp($row_id_pm['id_pm'], $id_pm))) {echo "selected=\"selected\"";} ?>><?php echo $row_id_pm['int_cod_ref_pm']?></option>
        <?php } ?>
      </select>

      <select class="busqueda selectsMini" name="fecha" id="fecha">
      <option value="0"<?php if (!(strcmp(0, $fecha))) {echo "selected=\"selected\"";} ?> >ANUAL</option>
         <?php  foreach($row_ano as $row_ano ) { ?>
          <option value="<?php echo $row_ano['anual']?>"<?php if (!(strcmp($row_ano['anual'], $fecha))) {echo "selected=\"selected\"";} ?>><?php echo $row_ano['anual']?></option>
      <?php } ?>  
      </select> 
  
    <input class="botonGMini" type="submit" name="Submit" value="FILTRO" onClick="if(consulta.id_pm.value=='0' && consulta.fecha.value=='0') { alert('DEBE SELECCIONAR UNA OPCION'); }"/>    
  </td>
...
<form action="delete_listado.php" method="get" name="seleccion">
  <table class="table table-bordered table-sm">
    ...
    <tr id="tr1">
      ...
      <td nowrap="nowrap" id="titulo4">MEZCLA N°</td>
      <td nowrap="nowrap" id="titulo4">REF N°</td>
